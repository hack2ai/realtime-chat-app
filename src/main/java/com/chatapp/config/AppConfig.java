 ...
 *
 * <p>Configuration is loaded once. Values may be supplied by a local
 * {@code config.properties} file, JVM system properties, or environment
 * variables. System properties and environment variables override the file,
 * which keeps deployment secrets out of source control.
 *
 * <p>Environment variable mapping is {@code CHATAPP_} + the key with dots
 * replaced by underscores and converted to upper case. For example,
 * {@code db.password} becomes {@code CHATAPP_DB_PASSWORD}.
 */
public final class AppConfig {
    ...
    private static String require(String key) {
        ...
        if (value == null || value.isBlank()) {
            String envKey = ENV_PREFIX + key.replace('.', '_').toUpperCase(Locale.ROOT);
