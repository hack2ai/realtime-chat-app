...
public final class ChatClientConnection implements AutoCloseable {
    ...
    public ChatClientConnection(Consumer<Envelope> eventListener, Consumer<Boolean> connectionListener) { this.eventListener = eventListener; this.connectionListener = connectionListener; }
    ...
    public synchronized void connect(String host, int port) throws IOException {
        ...
        Socket newSocket = createSocket(host.trim(), port);
        try {
    ...
    public CompletableFuture<LoginSuccessResponse> login(String usernameOrEmail, String password) {
        ...
        try { send(MessageType.C2S_LOGIN, new LoginRequest(usernameOrEmail, password)); }
        catch (IOException e) { clearPendingAuth(future); future.completeExceptionally(e); }
        return future;
    }
    ...
    public CompletableFuture<RegisterSuccessResponse> register(String username, String email, String password, String confirmPassword) {
        ...
        try { send(MessageType.C2S_REGISTER, new RegisterRequest(username, email, password, confirmPassword)); }
        catch (IOException e) { clearPendingAuth(future); future.completeExceptionally(e); }
        return future;
    }

    public void searchPrivateMessages(String query, int limit) throws IOException {
        if (query == null || query.isBlank()) throw new IllegalArgumentException("Search text cannot be empty.");
        send(MessageType.C2S_SEARCH_PRIVATE_MESSAGES, new SearchPrivateMessagesRequest(query.strip(), limit));
    }
    ...
    private boolean registerPendingAuth(CompletableFuture<?> future, Class<?> responseType) {
        synchronized (authLock) {
            if (pendingAuth != null && !pendingAuth.isDone()) { future.completeExceptionally(new IllegalStateException("Another authentication request is in progress.")); return false; }
            if (!running) { future.completeExceptionally(new IOException("Not connected.")); return false; }
            pendingAuth = future; pendingAuthType = responseType; return true;
        }
    }
    ...
    private void handleAuthResponse(Envelope envelope) {
        CompletableFuture<?> future; Class<?> responseType;
        synchronized (authLock) { future = pendingAuth; responseType = pendingAuthType; pendingAuth = null; pendingAuthType = null; }
        if (future == null || responseType == null) { safeEvent(envelope); return; }
        if (envelope.getType() == MessageType.S2C_LOGIN_FAILED || envelope.getType() == MessageType.S2C_REGISTER_FAILED) {
            ...
            future.completeExceptionally(new IllegalStateException(error == null ? "Authentication request failed." : error.getReason())); return;
        }
    ...
    private void readLoop() {
        try {
            while (running) {
                ...
                if (type == MessageType.S2C_LOGIN_SUCCESS || type == MessageType.S2C_LOGIN_FAILED || type == MessageType.S2C_REGISTER_SUCCESS || type == MessageType.S2C_REGISTER_FAILED) handleAuthResponse(envelope);
                else safeEvent(envelope);
            }
        } catch (IOException e) {
            if (running) {
                failPendingAuth(new IOException("Connection lost while waiting for authentication response.", e));
                safeEvent(codec.wrap(MessageType.S2C_ERROR, new AuthFailedResponse("Connection lost: network error")));
            }
