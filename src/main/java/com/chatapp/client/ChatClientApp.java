...
public final class ChatClientApp extends Application {
    ...
    private Parent loginView(Stage stage) {
        ...
        root.setAlignment(Pos.CENTER); root.setPadding(new Insets(50, 120, 50, 120)); root.getStyleClass().add("auth-shell");
        return root;
    }
    ...
    private VBox loginForm(Stage stage) {
        ...
        Runnable action = () -> {
            if (identity.getText().isBlank() || password.getText().isBlank()) { feedback.setText("Enter both fields."); return; }
            submit.setDisable(true); feedback.setText("Connecting…");
            runAsync(() -> { ensureConnected(); return connection.login(identity.getText().trim(), password.getText()); },
                    result -> { session = result; stage.setScene(new Scene(dashboard(stage), 1180, 760)); requestData(); },
                    error -> { feedback.setText(errorText(error)); submit.setDisable(false); });
        };
        submit.setOnAction(e -> action.run()); password.setOnAction(e -> action.run());
    ...
    private VBox registerForm() {
        ...
        submit.setOnAction(e -> {
            if (username.getText().isBlank() || email.getText().isBlank() || password.getText().isBlank() || confirm.getText().isBlank()) { feedback.setText("Complete every field."); return; }
            submit.setDisable(true); feedback.setText("Creating account…");
            runAsync(() -> { ensureConnected(); return connection.register(username.getText().trim(), email.getText().trim(), password.getText(), confirm.getText()); },
                    result -> { feedback.setText("Account created. You can now sign in."); submit.setDisable(false); },
                    error -> { feedback.setText(errorText(error)); submit.setDisable(false); });
        });
    ...
    private BorderPane dashboard(Stage stage) {
        BorderPane root = new BorderPane(); root.getStyleClass().add("dashboard");
        root.setTop(topBar(stage)); root.setLeft(sidebar()); root.setCenter(chatPane()); return root;
    }
    ...
    private HBox topBar(Stage stage) {
        ...
        Label online = new Label("● Connected"); online.getStyleClass().add("connected");
        Button logout = new Button("Sign out"); logout.setOnAction(e -> { try { connection.send(MessageType.C2S_LOGOUT, null); } catch (IOException ignored) {} connection.close(); stage.setScene(new Scene(loginView(stage), 980, 680)); });
        HBox bar = new HBox(14, title, spacer, online, account, logout); bar.setAlignment(Pos.CENTER_LEFT); bar.setPadding(new Insets(14, 18, 14, 18)); bar.getStyleClass().add("top-bar"); return bar;
    }
    ...
    private VBox sidebar() {
        ...
        groupView.setCellFactory(v -> new ListCell<>() { protected void updateItem(GroupSummary g, boolean empty) { super.updateItem(g, empty); setText(empty || g == null ? null : "# " + g.getName() + "  •  " + g.getMemberCount()); } });
        groupView.setOnMouseClicked(e -> { GroupSummary g = groupView.getSelectionModel().getSelectedItem(); if (g != null) openGroup(g); });

        Button create = new Button("+ New group"); create.setMaxWidth(Double.MAX_VALUE); create.setOnAction(e -> createGroup());
        Button join = new Button("Join by ID"); join.setMaxWidth(Double.MAX_VALUE); join.setOnAction(e -> joinGroup());
        VBox box = new VBox(8, section("PEOPLE"), people, section("GROUPS"), groupView, create, join);
        VBox.setVgrow(people, Priority.ALWAYS); VBox.setVgrow(groupView, Priority.ALWAYS); box.setPrefWidth(270); box.setPadding(new Insets(14)); box.getStyleClass().add("sidebar"); return box;
    }
    ...
    private VBox chatPane() {
        ...
        VBox header = new VBox(3, conversationTitle, hint); header.setPadding(new Insets(18));
        composer = new TextField(); composer.setPromptText("Write a message…"); Button send = primary("Send"); send.setOnAction(e -> sendMessage()); composer.setOnAction(e -> send.fire());
    ...
    private void openUser(UserSummary user) {
        selectedUser = user; selectedGroup = null; conversationTitle.setText(user.getUsername()); messages.getItems().clear();
        try { connection.send(MessageType.C2S_REQUEST_PRIVATE_HISTORY, new PrivateHistoryRequest(user.getUserId(), 100, 0)); } catch (IOException e) { system(e.getMessage()); }
    }
    private void openGroup(GroupSummary group) {
        selectedGroup = group; selectedUser = null; conversationTitle.setText("# " + group.getName()); messages.getItems().clear();
        try { connection.send(MessageType.C2S_REQUEST_GROUP_HISTORY, new GroupHistoryRequest(group.getGroupId(), 100, 0)); } catch (IOException e) { system(e.getMessage()); }
    }
    private void sendMessage() {
        String text = composer.getText().trim(); if (text.isEmpty()) return;
        try { if (selectedUser != null) connection.send(MessageType.C2S_PRIVATE_MESSAGE, new PrivateMessageRequest(selectedUser.getUserId(), text)); else if (selectedGroup != null) connection.send(MessageType.C2S_GROUP_MESSAGE, new GroupMessageRequest(selectedGroup.getGroupId(), text)); else { system("Select a conversation first."); return; } composer.clear(); }
        catch (IOException e) { system(e.getMessage()); }
    }
    ...
    private void onEnvelope(Envelope e) { Platform.runLater(() -> {
        switch (e.getType()) {
