...
public final class ChatClientApp extends Application {
    ...
    private VBox registerForm() {
        ...
        submit.setOnAction(e->{ if(username.getText().isBlank()||email.getText().isBlank()||password.getText().isBlank()||confirm.getText().isBlank()){feedback.setText("Complete every field.");return;}
            submit.setDisable(true); feedback.setText("Creating account…"); runAsync(()->{ensureConnected();return connection.register(username.getText().trim(),email.getText().trim(),password.getText(),confirm.getText()).join();},
                result->{feedback.setText("Account created. You can now sign in.");submit.setDisable(false);},error->{feedback.setText(errorText(error));submit.setDisable(false);}); });
    ...
    private HBox topBar(Stage stage){
        ...
        Button logout=new Button("Sign out"); logout.setOnAction(e->{try{if(connection.isConnected())connection.send(MessageType.C2S_LOGOUT,null);}catch(IOException ignored){}finally{connection.close();}session=null;selectedUser=null;selectedGroup=null;stage.setScene(styledScene(loginView(stage),980,680));});
    ...
    private void onEnvelope(Envelope envelope){Platform.runLater(()->{try{switch(envelope.getType()){
        ...
        case S2C_PRIVATE_HISTORY->{PrivateHistoryResponse r=codec.unwrap(envelope,PrivateHistoryResponse.class);if(selectedUser!=null&&selectedUser.getUserId()==r.getOtherUserId()){messages.getItems().clear();r.getMessages().forEach(this::privateMessage);}}
        case S2C_PRIVATE_MESSAGE->privateMessage(codec.unwrap(envelope,PrivateMessageEvent.class));
        case S2C_GROUP_HISTORY->{GroupHistoryResponse r=codec.unwrap(envelope,GroupHistoryResponse.class);if(selectedGroup!=null&&selectedGroup.getGroupId()==r.getGroupId()){messages.getItems().clear();r.getMessages().forEach(this::groupMessage);}}
        case S2C_GROUP_MESSAGE->groupMessage(codec.unwrap(envelope,GroupMessageEvent.class));
        case S2C_ERROR->system("Server rejected the request."); default->{}}
    }catch(RuntimeException e){system("Received an invalid server response.");}});}

    private void privateMessage(PrivateMessageEvent m){if(selectedUser==null||session==null||(m.getSenderId()!=selectedUser.getUserId()&&m.getReceiverId()!=selectedUser.getUserId()))return;String sender=m.getSenderId()==session.getUserId()?"You":selectedUser.getUsername();messages.getItems().add(sender+": "+m.getMessage());messages.scrollTo(messages.getItems().size()-1);}
    private void groupMessage(GroupMessageEvent m){if(selectedGroup==null||session==null||m.getGroupId()!=selectedGroup.getGroupId())return;String sender=m.getSenderId()==session.getUserId()?"You":m.getSenderUsername();messages.getItems().add(sender+": "+m.getMessage());messages.scrollTo(messages.getItems().size()-1);}
