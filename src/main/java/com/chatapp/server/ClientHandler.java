...
public class ClientHandler implements Runnable {
    ...
    private void cleanup(){if(!closed.compareAndSet(false,true))return;Thread watcher=sessionExpiryThread;if(watcher!=null)watcher.interrupt();ClientHandler handler=this;if(authenticatedUserId!=-1){authService.logout(sessionToken);server.removeClient(authenticatedUserId,handler);server.broadcastPresence(authenticatedUserId,authenticatedUsername,false);}try{socket.close();}catch(IOException ignored){}Thread writer=writerThread;if(writer!=null)writer.interrupt();outbound.clear();}
    private void closeConnection(){try{socket.close();}catch(IOException ignored){}cleanup();}
}