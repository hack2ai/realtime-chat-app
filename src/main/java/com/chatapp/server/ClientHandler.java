...
import com.chatapp.model.dto.NotificationDTO.NotificationEvent;
...
public class ClientHandler implements Runnable {
    ...
    private final ChatService chatService; private final GroupService groupService; private final MessageSearchService messageSearchService;
    ...
    public ClientHandler(Socket socket,ChatServer server,AuthenticationService authService,ChatService chatService,GroupService groupService){this.socket=socket;this.server=server;this.authService=authService;this.chatService=chatService;this.groupService=groupService;this.messageSearchService=new MessageSearchService(server::recordRateLimitedRequest);this.attachmentService=new AttachmentService(new com.chatapp.service.AttachmentStorageService(),new com.chatapp.database.AttachmentDAO(),new com.chatapp.database.UserDAO(),server::recordRateLimitedRequest);}
    ...
    private void messageLoop()throws IOException{while(!socket.isClosed()){final Envelope envelope;try{envelope=codec.read(in);}catch(EOFException e){logger.info("Client disconnected: {}",socket.getRemoteSocketAddress());return;}catch(SocketTimeoutException e){if(authenticatedUserId==-1)return;throw e;}catch(RuntimeException e){logger.warn("Invalid protocol message from {}; closing connection",socket.getRemoteSocketAddress());return;}if(envelope==null||envelope.getType()==null){sendError("Invalid message envelope.");continue;}server.recordRequest();try{dispatch(envelope);}catch(Exception e){logger.error("Error handling {} from {}: {}",envelope.getType(),socket.getRemoteSocketAddress(),e.getClass().getSimpleName());sendError("An internal error occurred processing your request.");}}}
    private void dispatch(Envelope envelope)throws Exception{switch(envelope.getType()){
        case PING->{if(PING_RATE_LIMITER.allow(buildAddressRateKey()))send(MessageType.PONG,null);else server.recordRateLimitedRequest();}
    ...
    private void requireAuth(HandlerAction action)throws Exception{if(authenticatedUserId==-1){sendError("You must log in before sending this message type.");return;}if(!REQUEST_RATE_LIMITER.allow("user:"+authenticatedUserId)){server.recordRateLimitedRequest();sendError(REQUEST_RATE_LIMIT_FAILURE);return;}if(!isSessionCurrent()){return;}action.run();}
    ...
    private void handleRegister(Envelope envelope)throws IOException{if(!REGISTRATION_RATE_LIMITER.allow(buildAddressRateKey())){server.recordRateLimitedRequest();send(MessageType.S2C_REGISTER_FAILED,new AuthFailedResponse(REGISTRATION_RATE_LIMIT_FAILURE));return;}RegisterRequest req=codec.unwrap(envelope,RegisterRequest.class);if(req==null){sendError("Invalid registration request.");return;}try{User created=authService.register(req.getUsername(),req.getEmail(),req.getPassword(),req.getConfirmPassword());send(MessageType.S2C_REGISTER_SUCCESS,new RegisterSuccessResponse(created.getId(),created.getUsername()));}catch(ValidationException e){send(MessageType.S2C_REGISTER_FAILED,new AuthFailedResponse(e.getMessage()));}}
    private void handleLogin(Envelope envelope)throws IOException{if(authenticatedUserId!=-1){sendError("This connection is already authenticated.");return;}LoginRequest req=codec.unwrap(envelope,LoginRequest.class);if(req==null){sendError("Invalid login request.");return;}String addressRateKey=buildAddressRateKey();if(!LOGIN_IP_RATE_LIMITER.allow(addressRateKey)){server.recordRateLimitedRequest();send(MessageType.S2C_LOGIN_FAILED,new AuthFailedResponse(IP_RATE_LIMIT_FAILURE));return;}String identifier=req.getUsernameOrEmail();String rateKey=buildRateKey(identifier);if(!LOGIN_RATE_LIMITER.allow(rateKey)){server.recordRateLimitedRequest();send(MessageType.S2C_LOGIN_FAILED,new AuthFailedResponse(RATE_LIMIT_FAILURE));return;}try{AuthenticationService.LoginResult result=authService.login(identifier,req.getPassword());authenticatedUserId=result.user().getId();authenticatedUsername=result.user().getUsername();sessionToken=result.sessionToken();if(!server.registerClient(authenticatedUserId,this)){authenticatedUserId=-1;authenticatedUsername=null;authService.logout(result.sessionToken());send(MessageType.S2C_LOGIN_FAILED,new AuthFailedResponse("This account is already connected."));return;}disableReadTimeout(socket);startSessionExpiryWatcher(result.expiresAt(),result.user().getId(),result.sessionToken());LOGIN_RATE_LIMITER.recordSuccess(rateKey);send(MessageType.S2C_LOGIN_SUCCESS,new LoginSuccessResponse(result.user().getId(),result.user().getUsername(),result.user().getRole().name(),result.sessionToken()));sendNotification("Welcome","You are now signed in securely.","INFO");logger.info("User authenticated from {}",socket.getRemoteSocketAddress());}catch(AuthenticationException e){LOGIN_RATE_LIMITER.recordFailure(rateKey);server.recordAuthenticationFailure();send(MessageType.S2C_LOGIN_FAILED,new AuthFailedResponse(e.getMessage()));}}
    static void disableReadTimeout(Socket socket)throws SocketException{socket.setSoTimeout(0);}
    ...
    private void sendNotification(String title,String message,String level){sendAsync(MessageType.S2C_NOTIFICATION,new NotificationEvent(title,message,level));}
    ...
    private void handleFileUpload(Envelope envelope)throws IOException,ValidationException{if(!UPLOAD_RATE_LIMITER.allow("user:"+authenticatedUserId)){server.recordRateLimitedRequest();sendError(UPLOAD_RATE_LIMIT_FAILURE);return;}PrivateFileUploadRequest req=codec.unwrap(envelope,PrivateFileUploadRequest.class);if(req==null){sendError("Invalid file upload request.");return;}PrivateFileEvent event=attachmentService.upload(authenticatedUserId,req.getReceiverId(),req.getFileName(),req.getContentType(),req.getDataBase64(),authenticatedUsername);ClientHandler recipient=server.getHandler(req.getReceiverId());if(recipient!=null)recipient.sendAsync(MessageType.S2C_PRIVATE_FILE,event);send(MessageType.S2C_PRIVATE_FILE,event);sendNotification("File sent","Your file was uploaded successfully.","SUCCESS");}
    ...
    private void cleanup(){if(closed.compareAndSet(false,true)){try{socket.close();}catch(IOException ignored){}}if(writerThread!=null)writerThread.interrupt();if(sessionExpiryThread!=null)sessionExpiryThread.interrupt();if(authenticatedUserId!=-1){int userId=authenticatedUserId;authenticatedUserId=-1;server.deregisterClient(userId,this);authService.logout(sessionToken);authenticatedUsername=null;sessionToken=null;}server.handlerClosed(this);}
}
