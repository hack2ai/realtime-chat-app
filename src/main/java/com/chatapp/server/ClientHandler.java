...
public class ClientHandler implements Runnable {
    ...
    @Override public void run() {
        try { in = new DataInputStream(socket.getInputStream()); out = new DataOutputStream(socket.getOutputStream()); messageLoop(); }
    ...
    private void handleLogin(Envelope envelope) throws IOException {
        ...
        try {
            ...
            if (!server.registerClient(authenticatedUserId, this)) { authenticatedUserId = -1; authenticatedUsername = null; authService.logout(result.sessionToken()); send(MessageType.S2C_LOGIN_FAILED, new AuthFailedResponse("This account is already connected.")); return; }
    ...
    private void handlePrivateMessage(Envelope envelope) throws IOException, ValidationException {
        ...
        if (recipient != null) { chatService.markDelivered(event.getReceiverId(), event.getMessageId()); event.setStatus("DELIVERED"); recipient.sendAsync(MessageType.S2C_PRIVATE_MESSAGE, event); send(MessageType.S2C_MESSAGE_DELIVERED, event); }
    ...
    private void handleTyping(Envelope envelope, MessageType type) throws IOException {
        PrivateMessageRequest req = codec.unwrap(envelope, PrivateMessageRequest.class);
        if (req == null || req.getReceiverId() <= 0 || req.getReceiverId() == authenticatedUserId || !chatService.userExists(req.getReceiverId())) { sendError("Invalid typing recipient."); return; }
