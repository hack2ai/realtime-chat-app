...
public class ClientHandler implements Runnable {
    ...
    private void messageLoop()throws IOException{while(!socket.isClosed()){final Envelope envelope;try{envelope=codec.read(in);}catch(EOFException e){logger.info("Client disconnected: {}",socket.getRemoteSocketAddress());return;}catch(SocketTimeoutException e){if(authenticatedUserId==-1)return;throw e;}catch(RuntimeException e){logger.warn("Invalid protocol message from {}; closing connection",socket.getRemoteSocketAddress());return;}if(envelope==null||envelope.getType()==null){sendError("Invalid message envelope.");continue;}try{dispatch(envelope);}catch(ValidationException e){String message=e.getMessage();sendError(message==null||message.isBlank()?"Invalid request.":message);}catch(Exception e){logger.error("Error handling {} from {}: {}",envelope.getType(),socket.getRemoteSocketAddress(),e.getClass().getSimpleName());sendError("An internal error occurred processing your request.");}}}
    ...
    private void handleLogin(Envelope envelope)throws IOException{if(authenticatedUserId!=-1){sendError("This connection is already authenticated.");return;}LoginRequest req=codec.unwrap(envelope,LoginRequest.class);if(req==null){sendError("Invalid login request.");return;}String addressRateKey=buildAddressRateKey();if(!LOGIN_IP_RATE_LIMITER.allow(addressRateKey)){server.recordAuthenticationFailure();send(MessageType.S2C_LOGIN_FAILED,new AuthFailedResponse(IP_RATE_LIMIT_FAILURE));return;}String identifier=req.getUsernameOrEmail();String rateKey=buildRateKey(identifier);if(!LOGIN_RATE_LIMITER.allow(rateKey)){server.recordAuthenticationFailure();send(MessageType.S2C_LOGIN_FAILED,new AuthFailedResponse(RATE_LIMIT_FAILURE));return;}try{AuthenticationService.LoginResult result=authService.login(identifier,req.getPassword());authenticatedUserId=result.user().getId();authenticatedUsername=result.user().getUsername();sessionToken=result.sessionToken();if(!server.registerClient(authenticatedUserId,this)){authenticatedUserId=-1;authenticatedUsername=null;authService.logout(result.sessionToken());send(MessageType.S2C_LOGIN_FAILED,new AuthFailedResponse("This account is already connected."));return;}socket.setSoTimeout(0);startSessionExpiryWatcher(result.expiresAt(),result.user().getId(),result.sessionToken());server.recordAuthenticationSuccess();LOGIN_RATE_LIMITER.recordSuccess(rateKey);send(MessageType.S2C_LOGIN_SUCCESS,new LoginSuccessResponse(result.user().getId(),result.user().getUsername(),result.user().getRole().name(),result.sessionToken()));sendNotification("Welcome","You are now signed in securely.","INFO");logger.info("User authenticated from {}",socket.getRemoteSocketAddress());}catch(AuthenticationException e){server.recordAuthenticationFailure();LOGIN_RATE_LIMITER.recordFailure(rateKey);send(MessageType.S2C_LOGIN_FAILED,new AuthFailedResponse(e.getMessage()));}}
    ...
    private void handlePrivateMessage(Envelope envelope)throws IOException,ValidationException{PrivateMessageRequest req=codec.unwrap(envelope,PrivateMessageRequest.class);if(req==null){sendError("Invalid private message request.");return;}PrivateMessageEvent event=chatService.sendPrivateMessage(authenticatedUserId,req.getReceiverId(),req.getMessage());ClientHandler recipient=server.getHandler(event.getReceiverId());boolean delivered=recipient!=null&&recipient.sendAsync(MessageType.S2C_PRIVATE_MESSAGE,event);if(delivered){chatService.markDelivered(event.getReceiverId(),event.getMessageId());event.setStatus("DELIVERED");}send(MessageType.S2C_PRIVATE_MESSAGE,event);}
    ...
    void send(MessageType type,Object payload)throws IOException{if(closed.get())throw new IOException("Client connection is closed");byte[] frame=codec.encode(codec.wrap(type,payload));if(!outbound.offer(frame)){server.recordOutboundQueueRejection();logger.warn("Outbound queue full for user {}; closing connection",authenticatedUserId);closeConnection();throw new IOException("Client outbound queue is full");}}
    boolean sendAsync(MessageType type,Object payload){if(closed.get())return false;try{byte[] frame=codec.encode(codec.wrap(type,payload));if(!outbound.offer(frame)){server.recordOutboundQueueRejection();logger.warn("Outbound queue full or client closed for user {}; closing connection",authenticatedUserId);cleanup();return false;}return true;}catch(IOException|RuntimeException e){logger.warn("Unable to prepare outbound message for user {}; closing connection ({})",authenticatedUserId,e.getClass().getSimpleName());cleanup();return false;}}
    private void writeNow(byte[] frame)throws IOException{if(out==null)throw new IOException("Client output stream is not initialized");out.writeInt(frame.length);out.write(frame);out.flush();}
    private void sendError(String message){int errors=protocolErrors.incrementAndGet();server.recordProtocolError();try{send(MessageType.S2C_ERROR,new AuthFailedResponse(message));}catch(IOException e){logger.debug("Unable to send error response to {}",socket.getRemoteSocketAddress());return;}if(errors>=MAX_PROTOCOL_ERRORS){logger.info("Protocol error budget exhausted for {}; closing connection",socket.getRemoteSocketAddress());closeConnection();}}
    private void handleLogout(){if(sessionToken!=null)authService.logout(sessionToken);closeConnection();}
    private void cleanup(){if(!closed.compareAndSet(false,true))return;Thread watcher=sessionExpiryThread;if(watcher!=null)watcher.interrupt();if(authenticatedUserId!=-1){authService.logout(sessionToken);server.deregisterClient(authenticatedUserId,this);}server.handlerClosed(this);try{socket.close();}catch(IOException ignored){}outbound.close();Thread writer=writerThread;if(writer!=null)writer.interrupt();}
