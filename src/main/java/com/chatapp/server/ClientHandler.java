...
public class ClientHandler implements Runnable {
    ...
    public ClientHandler(Socket socket, ChatServer server, AuthenticationService authService, ChatService chatService, GroupService groupService) {
        this.socket = socket; this.server = server; this.authService = authService; this.chatService = chatService; this.groupService = groupService;
    }
    ...
    @Override public void run() {
        try {
            ...
            messageLoop();
        } catch (IOException e) { logger.warn("I/O error on {}: {}", socket.getRemoteSocketAddress(), e.getMessage()); }
        finally { cleanup(); }
    }
    ...
    private void messageLoop() throws IOException {
        while (!socket.isClosed()) {
            ...
            catch (RuntimeException e) { logger.warn("Invalid protocol message from {}; closing connection: {}", socket.getRemoteSocketAddress(), e.getMessage()); return; }
            if (envelope == null || envelope.getType() == null) { sendError("Invalid message envelope."); continue; }
            try { dispatch(envelope); }
            catch (Exception e) { logger.error("Error handling {} from {}", envelope.getType(), socket.getRemoteSocketAddress(), e); sendError("An internal error occurred processing your request."); }
        }
    }
    ...
    private void dispatch(Envelope envelope) throws IOException {
        switch (envelope.getType()) {
            ...
            case C2S_REQUEST_GROUP_LIST -> requireAuth(() -> send(MessageType.S2C_GROUP_LIST, new GroupListResponse(groupService.list(authenticatedUserId))));
    ...
    private void handleRegister(Envelope envelope) throws IOException {
        ...
        try { User created = authService.register(req.getUsername(), req.getEmail(), req.getPassword(), req.getConfirmPassword()); send(MessageType.S2C_REGISTER_SUCCESS, new RegisterSuccessResponse(created.getId(), created.getUsername())); }
    ...
    private void handleTyping(Envelope envelope, MessageType type) throws IOException {
        PrivateMessageRequest req = codec.unwrap(envelope, PrivateMessageRequest.class);
        if (req == null || req.getReceiverId() <= 0 || req.getReceiverId() == authenticatedUserId) { sendError("Invalid typing recipient."); return; }
    ...
    private void handleCreateGroup(Envelope envelope) throws IOException, ValidationException {
        ...
        GroupSummary group = groupService.create(authenticatedUserId, req.getName());
        send(MessageType.S2C_GROUP_CREATED, new GroupCreatedResponse(group));
    }
    ...
    private void handleJoinGroup(Envelope envelope) throws IOException, ValidationException {
        ...
        groupService.join(authenticatedUserId, req.getGroupId());
        send(MessageType.S2C_GROUP_LIST, new GroupListResponse(groupService.list(authenticatedUserId)));
    }
    ...
    private void handleLeaveGroup(Envelope envelope) throws IOException, ValidationException {
        ...
        groupService.leave(authenticatedUserId, req.getGroupId());
        send(MessageType.S2C_GROUP_LIST, new GroupListResponse(groupService.list(authenticatedUserId)));
    }
    ...
    private void handleGroupMessage(Envelope envelope) throws IOException, ValidationException {
        ...
        for (int memberId : groupService.members(req.getGroupId())) {
            ClientHandler member = server.getHandler(memberId);
            if (member != null) member.sendAsync(MessageType.S2C_GROUP_MESSAGE, event);
        }
    }
    ...
    void send(MessageType type, Object payload) throws IOException {
        if (out == null) throw new IOException("Client output stream is not initialized.");
        codec.write(out, codec.wrap(type, payload));
    }
    void sendAsync(MessageType type, Object payload) {
        Thread.startVirtualThread(() -> { try { send(type, payload); } catch (IOException e) { logger.debug("Unable to push {} to user {}", type, authenticatedUserId); } });
    }
