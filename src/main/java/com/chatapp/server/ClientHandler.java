...
public class ClientHandler implements Runnable {
    ...
    private void requireAuth(HandlerAction action)throws Exception{if(authenticatedUserId==-1){sendError("You must log in before sending this message type.");return;}if(!REQUEST_RATE_LIMITER.allow("user:"+authenticatedUserId)){sendError(REQUEST_RATE_LIMIT_FAILURE);return;}action.run();}
    ...
    private void handleLogin(Envelope envelope)throws IOException{if(authenticatedUserId!=-1){sendError("This connection is already authenticated.");return;}LoginRequest req=codec.unwrap(envelope,LoginRequest.class);if(req==null){sendError("Invalid login request.");return;}String identifier=req.getUsernameOrEmail();String rateKey=buildRateKey(identifier);if(!LOGIN_RATE_LIMITER.allow(rateKey)){send(MessageType.S2C_LOGIN_FAILED,new AuthFailedResponse(RATE_LIMIT_FAILURE));return;}try{AuthenticationService.LoginResult result=authService.login(identifier,req.getPassword());authenticatedUserId=result.user().getId();authenticatedUsername=result.user().getUsername();sessionToken=result.sessionToken();if(!server.registerClient(authenticatedUserId,this)){authenticatedUserId=-1;authenticatedUsername=null;authService.logout(result.sessionToken());send(MessageType.S2C_LOGIN_FAILED,new AuthFailedResponse("This account is already connected."));return;}socket.setSoTimeout(0);LOGIN_RATE_LIMITER.recordSuccess(rateKey);send(MessageType.S2C_LOGIN_SUCCESS,new LoginSuccessResponse(result.user().getId(),result.user().getUsername(),result.user().getRole().name(),result.sessionToken()));logger.info("User '{}' authenticated from {}",authenticatedUsername,socket.getRemoteSocketAddress());}catch(AuthenticationException e){LOGIN_RATE_LIMITER.recordFailure(rateKey);send(MessageType.S2C_LOGIN_FAILED,new AuthFailedResponse(e.getMessage()));}}
    private String buildRateKey(String identifier){String normalized=identifier==null?"":identifier.strip().toLowerCase(Locale.ROOT);String address=socket.getInetAddress()==null?"unknown":socket.getInetAddress().getHostAddress();return address+"|"+normalized;}
