...
public class ClientHandler implements Runnable {
    ...
    void sendAsync(MessageType type,Object payload){if(closed.get())return;try{byte[] frame=codec.encode(codec.wrap(type,payload));if(!outbound.offer(frame)){server.recordOutboundQueueRejection();logger.warn("Outbound queue full or client closed for user {}; closing connection",authenticatedUserId);cleanup();}}catch(IOException|RuntimeException e){logger.warn("Unable to prepare outbound message for user {}; closing connection ({})",authenticatedUserId,e.getClass().getSimpleName());cleanup();}}
