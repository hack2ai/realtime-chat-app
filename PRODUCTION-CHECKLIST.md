...
Items marked **[x]** are controls enforced or validated by the repository's build/deployment configuration. They still need to be preserved by the production deployment. Items left unchecked require an operator or infrastructure owner to complete them.

## Application and transport

- [x] Run the server on a supported JDK 21 runtime for the built application.
...
- [x] Keep private keys and keystores outside Git and outside container build contexts.
- [ ] Configure database TLS where the database connection crosses a trust boundary.
- [x] Keep database public-key retrieval disabled unless there is a documented reason to enable it.
- [x] Keep the server bound to the intended private interface unless a public listener is explicitly required.
...
- [x] Do not ship default application accounts or passwords.
...
- [x] Run the server image as the non-root `chatapp` user.
- [x] Keep the container root filesystem read-only and provide write access only to the attachment volume and required temporary storage.
- [x] Keep Linux capabilities dropped unless a capability is explicitly required.
- [x] Keep `no-new-privileges` enabled.
- [x] Keep CPU, memory, process, and file-descriptor limits appropriate for the host capacity.
- [x] Keep database and server services on private networks; never expose the database port publicly.
...
- [x] Require CI to pass before production releases.
- [x] Review the generated SBOM for each release.
- [x] Verify artifact checksums before distribution.
- [x] Verify build provenance attestations in the release process.
- [x] Verify the published container digest before rollout.
- [x] Keep dependency automation enabled for Maven, GitHub Actions, and container base images.
